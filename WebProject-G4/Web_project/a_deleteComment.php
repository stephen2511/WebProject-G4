...
<script>
        function confirmDelete() {
            if (confirm("Are you sure you want to delete this recipe?")) {
                window.location.href = "a_deleteComment.php?confirm_delete_id=<?php echo $delete_id; ?>";
            }
        }
    </script>
...
<?php
    ...
    $query = "SELECT * FROM comment WHERE c_id = $delete_id";
    $result = mysqli_query($conn, $query);
    $admin = mysqli_fetch_assoc($result);

    if($admin) {
        // Delete the admin from the database
        $delete_query = "DELETE FROM comment WHERE c_id = $delete_id";
    ...
    echo "<script>window.location.href = 'a_viewComment.php';</script>";
} else {
    // Redirect to the home.php page if delete_id is not provided
    header("Location: a_viewComment.php");
    exit();
}
?>
